...
| Gemini CLI | Interactive default; `-p/--prompt` for non-interactive | No explicit attach; interactive via PTY/tmux; `--screen-reader` renders prompt | Stdin + `--prompt`; `-i` continues interactive (typed `?`) | Process stop | `--list-sessions`, `--resume`, `--delete-session` | Not confirmed; `--output-format json/stream-json` exists | `--prompt` run hit quota (capacity) |
